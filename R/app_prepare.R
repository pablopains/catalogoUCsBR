            ...
            tryCatch(
              {
                ...
                if(NROW(occ_tmp)>0)
                {
                  ...
                  # padronizar em dwc
                  {
                    ...
                    #                                                                ifelse(is.na(subspecies) & is.na(species) & is.na(genus) & !is.na(family), 'FAMILY',
                    #                                                                       ""))))))
                    #   )
                    
                    # occ[[source_data]] <- occ[[source_data]] %>%
                    #   dplyr::mutate(taxonRank = case_when(
                    #     !is.na(subspecies) & str_detect(tolower(scientificname), " var\\.") ~ "VARIETY",
                    #     !is.na(subspecies) & str_detect(tolower(scientificname), " subsp\\.") ~ "SUBSPECIES",
                    #     !is.na(subspecies) & str_detect(tolower(scientificname), " form\\.") ~ "FORM",
                    #     is.na(subspecies) & !is.na(species) & !is.na(genus) ~ "SPECIES",
                    #     is.na(subspecies) & is.na(species) & !is.na(genus) ~ "GENUS",
                    #     is.na(subspecies) & is.na(species) & is.na(genus) & !is.na(family) ~ "FAMILY",
                    #     TRUE ~ "sem taxom"
                    #   ))
                    ...
                    occ[[source_data]] <- occ[[source_data]] %>%
                      mutate(
                        rank_tag = str_to_lower(word(scientificname, 3)),
                        has_subsp = !(is.na(subspecies) | subspecies == ""),
                        has_sp    = !(is.na(species)      | species      == ""),
                        has_gen   = !(is.na(genus)        | genus        == ""),
                        has_fam   = !(is.na(family)       | family       == ""),
                        taxonRank = case_when(
                          has_subsp & rank_tag == "var."   ~ "VARIETY",
                          has_subsp & rank_tag == "subsp." ~ "SUBSPECIES",
                          has_subsp & rank_tag == "form."  ~ "FORM",
                          !has_subsp & has_sp  & has_gen   ~ "SPECIES",
                          !has_subsp & !has_sp & has_gen   ~ "GENUS",
                          !has_subsp & !has_sp & !has_gen  & has_fam ~ "FAMILY",
                          TRUE ~ ""  # ou NA_character_
                        )
                      ) %>%
                      select(-rank_tag, -has_subsp, -has_sp, -has_gen, -has_fam)
                    
                    # x$taxonRank %>% unique()
                    
                    occ[[source_data]] <- occ[[source_data]] %>%
                      mutate(
                        # reconstrói scientificname de acordo com o nível taxonômico
                        scientificname = case_when(
                          taxonRank %in% c("GENUS", "FAMILY") ~ word(scientificname, 1),
                          taxonRank == "SPECIES"   ~ str_c(word(scientificname, 1), word(scientificname, 2), sep = " "),
                          taxonRank == "VARIETY"   ~ str_c(word(scientificname, 1), word(scientificname, 2), "var.",   word(scientificname, 4), sep = " "),
                          taxonRank == "SUBSPECIES"~ str_c(word(scientificname, 1), word(scientificname, 2), "subsp.", word(scientificname, 4), sep = " "),
                          taxonRank == "FORM"      ~ str_c(word(scientificname, 1), word(scientificname, 2), "form.",  word(scientificname, 4), sep = " "),
                          # tudo o mais vira NA
                          TRUE ~ NA_character_
                        )
                      )
                    
                    # x$scientificname %>% unique()
                                                                         ...
                                                                         if(NROW(occ[['all']])>0)
                                                                         {
                                                                           ...
                                                                           if(NROW(r_tmp)>0){
                                                                             ...
                                                                             for(i in 1:NROW(r_tmp))
                                                                             {
                                                                               ...
                                                                               index <- endsWith(r_tmp$Ctrl_key_family_recordedBy_recordNumber[i], "_")
                                                                               
                                                                               if(index==TRUE){
                                                                                 r_tmp$Ctrl_key_family_recordedBy_recordNumber[i] <-  paste0(r_tmp$Ctrl_key_family_recordedBy_recordNumber[i], r_tmp$Ctrl_occurrenceID[i])
                                                                               }
                                                                               
                                                                               
                                                                               r_tmp$Ctrl_scientificName[i] <- ifelse(
                                                                                 is.na(r_tmp$Ctrl_scientificName[i]) | trimws(r_tmp$Ctrl_scientificName[i]) == "",
                                                                                 paste0(r_tmp$Ctrl_family[i], " indeterminada "),
                                                                                 r_tmp$Ctrl_scientificName[i]
                                                                               )
                                                                               
                                                                               
                                                                             }
                                                                             
                                                                             # r_tmp$Ctrl_scientificName <- ifelse(is.na(r_tmp$Ctrl_scientificName) | r_tmp$Ctrl_scientificName =="", paste0(r_tmp$Ctrl_family, " indeterminada: "),r_tmp$Ctrl_scientificName )
                                                                             
                                                                             # Converter para character (se necessário)
                                                                             # r_tmp$Ctrl_scientificName <- as.character(r_tmp$Ctrl_scientificName)
                                                                             # r_tmp$Ctrl_family <- as.character(r_tmp$Ctrl_family)
                                                                             
                                                                             # Verificar NAs em Ctrl_family e substituir por texto padrão
                                                                             # r_tmp$Ctrl_family[is.na(r_tmp$Ctrl_family)] <- "Família desconhecida"
                                                                             
                                                                             # Aplicar a condição com trimws()
                                                                             
                                                                             
                                                                             # r_tmp$Ctrl_scientificName <- ""
                                                                             
                                                                             
                                                                             # r_tmp <- r_tmp %>%
                                                                             #   # Criar uma coluna temporária para priorizar REFLORA
                                                                             #   dplyr::mutate(priority = ifelse(Ctrl_bibliographicCitation == "reflora", 1, 2)) %>%
                                                                             #   # Ordenar conforme sua necessidade, mas com prioridade para REFLORA
                                                                             #   dplyr::arrange(Ctrl_key_family_recordedBy_recordNumber, priority) %>%
                                                                             #   # Manter apenas o primeiro registro de cada espécie
                                                                             #   dplyr::distinct(Ctrl_scientificName, .keep_all = TRUE) %>%
                                                                             #   # Remover a coluna temporária
                                                                             #   dplyr::select(-priority)
                                                                             # # write.csv(r_tmp2, "C:\\temps\\res_teste.csv", row.names = FALSE,  fileEncoding = "UTF-8", na = "")
